...
Provides comprehensive validation for Streamlit form inputs including numeric ranges,
geographic coordinates, power ratings, and energy system parameters.
...
class InputValidator:
    ...
    def validate_positive_number(
        ...
        """Validate that a number is positive (optionally allowing zero).
        ...
        if allow_zero and value < 0:
            ...
            return (
                False, 
                f"❌ {field_name} must be greater than 0. Current value: {value}"
            )
        return True, ""
    ...
    def validate_efficiency(value: float, field_name: str) -> Tuple[bool, str]:
        """Validate efficiency input (0-100%) with warning for very high values.
        ...
        if value < 0 or value > 100:
            return (
                False, 
                f"❌ {field_name} must be between 0% and 100%. Current value: {value}%"
            )
        if value > 95:
            return (
                True, 
                f"⚠️ Warning: {field_name} is very high ({value}%). "
                "Please verify this value."
            )
        return True, ""
    
    @staticmethod
    def validate_geographic_coordinate(
        latitude: float, 
        longitude: float
    ) -> Tuple[bool, str]:
        ...
        if latitude < -90 or latitude > 90:
            return (
                False, 
                f"❌ Latitude must be between -90° and 90°. Current value: {latitude}°"
            )
        if longitude < -180 or longitude > 180:
            return (
                False, 
                f"❌ Longitude must be between -180° and 180°. "
                f"Current value: {longitude}°"
            )
        return True, ""
    ...
    def validate_power_rating(
        ...
        if value < 0:
            return (
                False, 
                f"❌ {field_name} cannot be negative. Current value: {value} kW"
            )
        if value > max_reasonable:
            return (
                True, 
                f"⚠️ Warning: {field_name} is very high ({value} kW). "
                "Please verify this value."
            )
        if value == 0:
            return (
                True, 
                f"⚠️ Warning: {field_name} is set to 0 kW. "
                "This may result in no energy generation/consumption."
            )
        return True, ""
    ...
    def validate_angle(
        ...
        return InputValidator.validate_numeric_range(
            value, min_angle, max_angle, f"{field_name} (degrees)"
        )
...
def validate_location_selection(
    ...
    if location not in available_locations:
        return (
            False, 
            f"❌ Selected location '{location}' is not available in the database"
        )
    return True, ""
...
def validate_energy_system_inputs(
    ...
    if power_rating is not None:
        results.append(
            InputValidator.validate_power_rating(power_rating, "Power Rating")
        )
