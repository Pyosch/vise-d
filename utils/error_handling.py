...
def handle_database_errors(func: Callable) -> Callable:
    ...
    def wrapper(*args, **kwargs):
        try:
            ...
            st.info("💡 **Alternative**: Try selecting a different location or contact support.")
            return None
...
def handle_data_processing_errors(func: Callable) -> Callable:
    ...
    def wrapper(*args, **kwargs):
        try:
            ...
            logger.error(f"Unexpected data processing error in {func.__name__}: {str(e)}")
    ...
    try:
        if operation == "read":
            ...
            if path.exists() and not path.is_file():
                st.error(f"📁 **Cannot Write**: `{file_path}` exists but is not a file")
                return False
...
def display_error_summary(errors: Dict[str, Any]) -> None:
    """
    Display a comprehensive error summary
...
def show_loading_with_progress(message: str, progress_callback: Optional[Callable] = None):
    """
