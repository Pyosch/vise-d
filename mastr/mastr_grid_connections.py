...
import pickle
from mastr_preprocessing import fetch_grid_connections, df_to_gdf, add_centroids
...
df_grid_connections = fetch_grid_connections()
#gdf_grid_connections = df_to_gdf(df_grid_connections)
#gdf_grid_connections = add_centroids(gdf_grid_connections)
...
# plt.show()
# def prepare_grid_connections_data(location='Essen', start_pv="2015-07-07 00:00:00", end_pv="2015-07-07 23:45:00"):
...
#             return df_grid_connections, city_district
...
#     gdf_grid_connections, city_district = prepare_grid_connections_data()
import osmnx as ox
import geopandas as gpd
...
def prepare_grid_connections_data(location='Essen', start_pv="2015-07-07 00:00:00", end_pv="2015-07-07 23:45:00"):
    try:
        df_grid_connections = fetch_grid_connections().head(20)

        # Use some column that contains location names (city names, etc.)
        # Example: 'NetzanschlusspunktBezeichnung' or 'NameDerTechnischenLokation'
        address_column = 'NetzanschlusspunktBezeichnung'
        coords = []

        for loc in df_grid_connections[address_column].fillna('').unique():
            try:
                gdf = ox.geocode_to_gdf(loc + ', Germany')
                point = gdf.geometry.centroid.iloc[0]
                coords.append((loc, point.y, point.x))  # name, lat, lon
            except:
                coords.append((loc, None, None))  # in case geocoding fails

        # Create DataFrame from results and merge
        coords_df = pd.DataFrame(coords, columns=[address_column, 'latitude', 'longitude'])
        df_grid_connections = df_grid_connections.merge(coords_df, on=address_column, how='left')

        # Drop rows without coordinates
        df_grid_connections.dropna(subset=['latitude', 'longitude'], inplace=True)

        # Convert to GeoDataFrame
        gdf_grid_connections = gpd.GeoDataFrame(
            df_grid_connections,
            geometry=gpd.points_from_xy(df_grid_connections.longitude, df_grid_connections.latitude),
            crs="EPSG:4326"
        )

        # Get city district for display/centering
        city_district = ox.geocode_to_gdf([location])

        return gdf_grid_connections, city_district
